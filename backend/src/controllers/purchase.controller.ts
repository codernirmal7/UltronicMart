...
import { sendOrderConfirmationEmail } from "../nodemailer/email.nodemailer";
...
const purchaseProduct = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    for (const update of productsToUpdate) {
      ...
      if (product && product.stock > 0) {
        ...
        res
          .status(404)
          .json({
            message: "Error while updating product stocks and purchaseCount.",
          });
        return;
      }
    }
    ...
    res
      .status(500)
      .json({ message: "Error processing purchase", error: error });
  }
};
